...
bool SteerLib::GJK_EPA::GJK(const std::vector<Util::Vector>& _shapeA, const std::vector<Util::Vector>& _shapeB, std::vector<Util::Vector>& simplex)
{
	...
	{
		auto temp = this->shortest_projection(A_minus_B, v); // get point in A_minus_B that has the shortest projection on v
		...
		else if (simplex.size() > 2)
		{
			...
			if(!close_enough)
			{
				...
				simplex.push_back(std::get<1>(temp[0]));
				auto temp2 = this->shortest_projection2(simplex, origin);// update v as the origin's shortest projection point on all W's edges that is also inside W
				v = std::get<0>(temp2);
			}
		}
	}
...
bool SteerLib::GJK_EPA::triangle_contain_point(std::vector<Util::Vector>& W, Util::Vector& v)
{
	...
	b = W[1];
	c = W[2];
	if ((sign(this->determinant(a,b,c)) ==  sign(this->determinant(a,b,v)) && sign(this->determinant(a,b,v)) == sign(this->determinant(b,c,v)) && sign(this->determinant(b,c,v)) == sign(this->determinant(c,a,v)) && sign(this->determinant(c,a,v)) == sign(this->determinant(a,b,c))) || sign(this->determinant(c,a,v)) == 0 || sign(this->determinant(a,b,v)) == 0 || sign(this->determinant(b,c,v)) == 0 )
		return true;
	else 
		return false;
}
...
std::pair<float,std::vector<Util::Vector>>  SteerLib::GJK_EPA::EPA(const std::vector<Util::Vector>& _shapeA, const std::vector<Util::Vector>& _shapeB, std::vector<Util::Vector>& simplex)
{
	...
	queue.insert( this->ortho_projection_point2(c, a, origin));
	...
	while (!converge)
	{
		...
		if(converge)
		{
			return std::make_pair( std::sqrt(len2), dir);
		}
		else
		{
			queue.insert( this->ortho_projection_point2(left_node, expand_node, origin));
			queue.insert( this->ortho_projection_point2(right_node, expand_node, origin));
			queue.erase(min);
		}
	}
}
...
bool SteerLib::GJK_EPA::intersect(float& return_penetration_depth, Util::Vector& return_penetration_vector, const std::vector<Util::Vector>& _shapeA, const std::vector<Util::Vector>& _shapeB)
{
	...
	is_colliding = this->GJK(_shapeA, _shapeB, simplex);
	if(is_colliding == true)
	{
		std::make_pair(return_penetration_depth, return_penetration_vector) = this->EPA(_shapeA, _shapeB, simplex);
		return true;
	}
	else	
