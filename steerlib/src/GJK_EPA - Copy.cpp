...
bool SteerLib::GJK_EPA::GJK(const std::vector<Util::Vector>& _shapeA, const std::vector<Util::Vector>& _shapeB, std::vector<Util::Vector>& simplex)
{
	...
	std::vector<Util::Vector> test;
	Util::Vector t1, t2, t3;
	t1.x = -1; t1.z = -3; t2.x = 5; t2.z = -1; t3.x = -2; t3.z = -3;
	test.push_back(t1);
	test.push_back(t2);
	test.push_back(t3);
	bool ttt = triangle_contain_point(test, origin);
	...
	//			std::cerr << "repeat \n";
				...
				std::cout << u << ' ' << v.length() << ' ' << std::sqrt(std::get<1>(temp2)) <<' ' << std::get<0>(temp2).x << ' ' << std::get<0>(temp2).z << std::endl;
				std::cout << std::get<1>(temp[0]).x << ' ' << std::get<1>(temp[0]).z << std::endl;
...
//				it = std::find(simplex.begin(), simplex.end(), std::get<2>(temp2));
...
std::tuple<Util::Vector,float,Util::Vector> SteerLib::GJK_EPA::shortest_projection2(std::vector<Util::Vector>& W, Util::Vector& v)
{
	...
	std::cout << std::get<0>(temp[0]).x << ' ' << std::get<0>(temp[0]).z << std::endl;
	int n=0;
...
std::vector<Util::Vector> SteerLib::GJK_EPA::A_min_B(const std::vector<Util::Vector>& _shapeA, const std::vector<Util::Vector>& _shapeB)
{
	...
	for(auto i = _shapeA.begin(); i != _shapeA.end(); i++ )
		for(auto j = _shapeB.begin(); j != _shapeB.end(); j++)
		{
			...
			//std::cout << j->x << " " << j->z << std::endl;
			a_b = (*i) - (*j);
			std::cout << a_b.x << " " << a_b.z << std::endl;
			A_B.push_back(a_b);
		}
	return A_B;
